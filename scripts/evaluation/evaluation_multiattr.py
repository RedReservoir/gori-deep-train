...
def compute_multiattr_logits_probs_subprocess(
    rank,
    ...
    pred_multiattr_logits_temp_filename,
    target_multiattr_probs_temp_filename,
    multiattr_weights_temp_filename
):
    ...
    multiattr_key_ddict = module_transform_pymodule.multiattr_key_ddict
    ...
    # Prepare accumulated data buffers

    pred_multiattr_logit_tenn_dict = {
        supattr_name: torch.empty(
            size=(dataset_split_idx_chunk.shape[0], supattr_size),
            dtype=torch.float,
            device=device
        )
    ...
    target_multiattr_prob_tenn_dict = {
        supattr_name: torch.empty(
            size=(dataset_split_idx_chunk.shape[0], supattr_size),
            dtype=torch.float,
            device=device
    ...
    for data_batch in chunk_dataloader:
        ...
        with torch.no_grad():
            ...
            for supattr_name in multiattr_metadata.supattr_name_list:
                ...
                batch_pred_multiattr_logit_ten = data_batch[multiattr_key_ddict[supattr_name]["pred_multiattr_logit_ten_key"]]
                batch_target_multiattr_prob_ten = data_batch[multiattr_key_ddict[supattr_name]["target_multiattr_prob_ten_key"]]
                batch_multiattr_weight_ten = data_batch[multiattr_key_ddict[supattr_name]["multiattr_weight_ten_key"]]

                pred_multiattr_logit_tenn_dict[supattr_name][start_idx:end_idx] = batch_pred_multiattr_logit_ten[:]
                target_multiattr_prob_tenn_dict[supattr_name][start_idx:end_idx] = batch_target_multiattr_prob_ten[:]
                multiattr_weight_tenn_dict[supattr_name][start_idx:end_idx] = batch_multiattr_weight_ten[:]
    ...
    logger.info("Saving predicted multi-attribute logits to {:s}".format(pred_multiattr_logits_temp_filename))
    torch.save(pred_multiattr_logit_tenn_dict, pred_multiattr_logits_temp_filename)

    logger.info("Saving target multi-attribute probabilities to {:s}".format(target_multiattr_probs_temp_filename))
    torch.save(target_multiattr_prob_tenn_dict, target_multiattr_probs_temp_filename)

    logger.info("Saving multi-attribute weights to {:s}".format(multiattr_weights_temp_filename))
    torch.save(multiattr_weight_tenn_dict, multiattr_weights_temp_filename)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--resume",
        help="resumes evaluation from current state",
        action="store_true"
    )
    
    parser.add_argument(
        "--reset",
        help="resets evaluation and starts anew",
        action="store_true"
    )
    ...
    logger.info("Load metadata settings")
    ...
    #
    # Run multi-attribute inference subprocesses
    #


    os.makedirs(eval_results_data_dirname, exist_ok=True)

    pred_multiattr_logits_dirname = os.path.join(eval_results_data_dirname, "pred_multiattr_logits")
    target_multiattr_probs_dirname = os.path.join(eval_results_data_dirname, "target_multiattr_probs")
    multiattr_weights_dirname = os.path.join(eval_results_data_dirname, "multiattr_weights")

    os.makedirs(pred_multiattr_logits_dirname, exist_ok=True)
    os.makedirs(target_multiattr_probs_dirname, exist_ok=True)
    os.makedirs(multiattr_weights_dirname, exist_ok=True)
    ...
    for dataset_name in dataset_name_list:
        ...
        pred_multiattr_logits_dataset_dirname = os.path.join(pred_multiattr_logits_dirname, dataset_name)
        target_multiattr_probs_dataset_dirname = os.path.join(target_multiattr_probs_dirname, dataset_name)
        multiattr_weights_dataset_dirname = os.path.join(multiattr_weights_dirname, dataset_name)

        os.makedirs(pred_multiattr_logits_dataset_dirname, exist_ok=True)
        os.makedirs(target_multiattr_probs_dataset_dirname, exist_ok=True)
        os.makedirs(multiattr_weights_dataset_dirname, exist_ok=True)
        ...
        for split_str in ["train", "val", "test"]:
            ...
            pred_multiattr_logits_temp_filename_list = [
                os.path.join(pred_multiattr_logits_dataset_dirname, "{:s}__{:d}.pt".format(split_str, rank))
                for rank in range(world_size)
            ]

            target_multiattr_probs_temp_filename_list = [
                os.path.join(target_multiattr_probs_dataset_dirname, "{:s}__{:d}.pt".format(split_str, rank))
                for rank in range(world_size)
            ]

            multiattr_weights_temp_filename_list = [
                os.path.join(multiattr_weights_dataset_dirname, "{:s}__{:d}.pt".format(split_str, rank))
                for rank in range(world_size)
            ]
            ...
            for rank, proc_args in enumerate(proc_args_zip):
                ...
                proc = multiprocessing.Process(
                    target=compute_multiattr_logits_probs_subprocess,
                    args=(
                        rank,
                        command_args,
            ...
            for proc_zidx, proc in enumerate(procs):
                if proc.exitcode != 0:
                    raise ValueError("Subprocess {:s} [{:d}] exited with code {:d}".format(
                        "compute_multiattr_logits_probs_subprocess",
                        proc_zidx,
                        proc.exitcode
                    ))
            ...
            # Join all predicted multi-attribute logits

            logger.info("Join all predicted multi-attribute logits")

            pred_multiattr_logits_temp_tenn_dict_list = []

            for pred_multiattr_logits_temp_filename in pred_multiattr_logits_temp_filename_list:

                pred_multiattr_logits_temp_tenn_dict = torch.load(pred_multiattr_logits_temp_filename, map_location=device)
                pred_multiattr_logits_temp_tenn_dict_list.append(pred_multiattr_logits_temp_tenn_dict)
                os.remove(pred_multiattr_logits_temp_filename)

                logger.info("Deleted temp predicted multi-attribute logits {:s}".format(pred_multiattr_logits_temp_filename))

            pred_multiattr_logits_joined_tenn_dict = {
                supattr_name: torch.cat([
                    pred_multiattr_logits_temp_tenn_dict[supattr_name]
                    for pred_multiattr_logits_temp_tenn_dict in pred_multiattr_logits_temp_tenn_dict_list
                ])
                for supattr_name in multiattr_metadata.supattr_name_list
            }

            pred_multiattr_logits_joined_filename = os.path.join(pred_multiattr_logits_dataset_dirname, "{:s}.pt".format(split_str))
            torch.save(pred_multiattr_logits_joined_tenn_dict, pred_multiattr_logits_joined_filename)

            del pred_multiattr_logits_temp_tenn_dict_list
            del pred_multiattr_logits_joined_tenn_dict

            logger.info("Saved joined predicted multi-attribute logits {:s}".format(pred_multiattr_logits_joined_filename))

            # Join all target multi-attribute probabilities

            logger.info("Join all target multi-attribute probabilities")

            target_multiattr_probs_temp_tenn_dict_list = []

            for target_multiattr_probs_temp_filename in target_multiattr_probs_temp_filename_list:

                target_multiattr_probs_temp_tenn_dict = torch.load(target_multiattr_probs_temp_filename, map_location=device)
                target_multiattr_probs_temp_tenn_dict_list.append(target_multiattr_probs_temp_tenn_dict)
                os.remove(target_multiattr_probs_temp_filename)

                logger.info("Deleted temp target multi-attribute probabilities {:s}".format(target_multiattr_probs_temp_filename))

            target_multiattr_probs_joined_tenn_dict = {
                supattr_name: torch.cat([
                    target_multiattr_probs_temp_tenn_dict[supattr_name]
                    for target_multiattr_probs_temp_tenn_dict in target_multiattr_probs_temp_tenn_dict_list
                ])
                for supattr_name in multiattr_metadata.supattr_name_list
            }

            target_multiattr_probs_joined_filename = os.path.join(target_multiattr_probs_dataset_dirname, "{:s}.pt".format(split_str))
            torch.save(target_multiattr_probs_joined_tenn_dict, target_multiattr_probs_joined_filename)

            del target_multiattr_probs_temp_tenn_dict_list
            del target_multiattr_probs_joined_tenn_dict

            logger.info("Saved joined target multi-attribute probabilities {:s}".format(target_multiattr_probs_joined_filename))

            # Join all multi-attribute weights

            logger.info("Join all multi-attribute weights")

            multiattr_weights_temp_tenn_dict_list = []

            for multiattr_weights_temp_filename in multiattr_weights_temp_filename_list:

                multiattr_weights_temp_tenn_dict = torch.load(multiattr_weights_temp_filename, map_location=device)
                multiattr_weights_temp_tenn_dict_list.append(multiattr_weights_temp_tenn_dict)
                os.remove(multiattr_weights_temp_filename)

                logger.info("Deleted temp multi-attribute weights {:s}".format(multiattr_weights_temp_filename))

            multiattr_weights_joined_tenn_dict = {
                supattr_name: torch.cat([
                    multiattr_weights_temp_tenn_dict[supattr_name]
                    for multiattr_weights_temp_tenn_dict in multiattr_weights_temp_tenn_dict_list
                ])
                for supattr_name in multiattr_metadata.supattr_name_list
            }

            multiattr_weights_joined_filename = os.path.join(multiattr_weights_dataset_dirname, "{:s}.pt".format(split_str))
            torch.save(multiattr_weights_joined_tenn_dict, multiattr_weights_joined_filename)
            
            del multiattr_weights_temp_tenn_dict_list
            del multiattr_weights_joined_tenn_dict

            logger.info("Saved joined multi-attribute weights {:s}".format(multiattr_weights_joined_filename))
