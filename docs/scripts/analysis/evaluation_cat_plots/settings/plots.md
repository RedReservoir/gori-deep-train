...
```
  ...
  - `cat_conf_mat`: Confusion matrix plot settings. Can contain multiple elements.
    - `group_names`: List of group names for which to plot the confusion matrix.
    - `pn_normalized`: If `true`, the confusion matrix will be PN-normalized (normalized wrt. number of positive and negative samples).

  - `cat_conf_aggs`: Confusion matrix aggregates plots. Can contain multiple elements.
  ...
  - `cat_conf_aggs`: Confusion matrix metric plots (bar plots). Can contain multiple elements.
