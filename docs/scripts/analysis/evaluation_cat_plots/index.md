...
The `data` directory contains the results of the analysis pipeline, comprising multiple plots:
  ...
  - `cat_conf_mat_plots`: Confusion matrix plots.
  - `cat_conf_aggs_plots`: Confusion matrix aggregates plots. Equivalent to mini-confusion matrices for each particular category.
  - `cat_conf_metric_plots`: Confusion matrix metric plots. Showcases usual confusion matrix metrics such as Precision, Recall, Accuracy, F1-Score, and Fβ-Score, both averaged and per category.
